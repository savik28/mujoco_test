...
unsigned int mujoco_init()
{
    ...
    double start_pose[8]    = {0.1,-1,1,-2.5,-1,0.2,-0.05,0.05};//{0.1,-1,1,-2.5,-1,0,-0.05,0.05};{0,0,0,0,0,0,0,0}
    double pos_set_point[8] = {0.3,-1.45,1.45,-3.14,-1.46,0.9,-0.04,0.04};
...
unsigned int chain_creator()
{
    ...
    bool chain_done, right_done, left_done;
    KDL::Chain left_chain, right_chain;
    chain_done = my_tree.getChain("base_link","gripper_base_link",my_chain);
    left_done  = my_tree.getChain("gripper_base_link","gripper_finger_left_link",left_chain);
    right_done = my_tree.getChain("gripper_base_link","gripper_finger_right_link",right_chain);
